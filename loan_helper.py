...
class LoanDataset(data.Dataset):
    ...
    def __init__(self, csv_file):
        ...
        #loans_df = pd.get_dummies(loans_df.loc[:,'loan_status'])

        self.train_labels = y_train.values
        self.test_labels = y_test.values
        status_list = list(set(self.test_labels))

        print(csv_file, "train", len(self.train_data),"test",status_list)
